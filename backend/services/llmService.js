      ...
      "expenses": [
        {
          ...
          "category": "One of 'transport', 'food', 'tickets', 'shopping', 'lodging', 'tickets', 'entertainment', 'other'",
 ...
 * 调用大语言模型 API 来生成旅行计划
 * @param {string} userInput - 用户的原始输入
 * @param {string} apiKey - 从用户 profile 获取的 API Key
 * @returns {Promise<object>} - 解析后的 JSON 对象，代表旅行计划
 */
const callLLMApi = async (userInput, apiKey) => {
    if (!apiKey) {
        throw new Error('API Key is required to call the language model.');
    }
